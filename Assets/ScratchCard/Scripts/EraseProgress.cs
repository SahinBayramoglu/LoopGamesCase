                ...
                if (progressAccuracy == ProgressAccuracy.High)
                {
                    ...
                    if (sampleSourceTexture)
                    {
                        ...
                        /*for (var i = 0; i < pixelsBuffer.Length; i += bitsPerPixel)
                        {
                            ...
                            totalAlpha = totalAlpha + sourceAlpha;
                            // progress += pixelsBuffer[i] / 255f * sourceAlpha;
                            for (int j = 0; j < pixelsBuffer.Length; j += 4)
                            {
                                byte r = pixelsBuffer[j];     // Kırmızı kanal
                                byte g = pixelsBuffer[j + 1]; // Yeşil kanal
                                byte b = pixelsBuffer[j + 2]; // Mavi kanal
                                byte a = pixelsBuffer[j + 3]; // Alpha kanal

                                // Kırmızı kanalı normalize edip kullanma örneği
                                progress += r / 255f * sourceAlpha;
                            }
                        }*/
